...
- `database.py`: conexión SQLite, migración v1 y reconciliación de jobs huérfanos.
...
- `server.py`: HTTP de loopback, API JSON, estáticos y artifacts.
...
El guion/audio son fixtures deterministas para validar etapas; importación real se cierra cuando exista un ejemplo de guion. Cancelación multimedia cooperativa y empaquetado en ejecutable son mejoras posteriores a la primera evaluación de la interfaz.
